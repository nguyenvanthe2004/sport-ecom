...
class CartController {
  async getMyCart(req, res) {
    try {
      ...
      const cart = await Cart.findOne({ userId }).populate({
        path: "cartItems",
        populate: { path: "variantId", select: "name price color size" },
      });
  ...
  async addToCart(req, res) {
    try {
      ...
      let existingItem = await CartItem.findOne({ _id: { $in: cart.cartItems }, variantId });
      if (existingItem) {
      ...
      await cart.save();
       // Cập nhật tổng tiền
      const allItems = await CartItem.find({ _id: { $in: cart.cartItems } }).populate('variantId', 'price');
      cart.totalPrice = allItems.reduce((sum, i) => sum + i.variantId.price * i.quantity, 0);
      await cart.save();
  ...
  try {
    ...
    if (!quantity && !variantId) {
      return res.status(400).json({ message: "Vui lòng nhập ít nhất một trường để cập nhật" });
    }
    ...
    const updatedItem = await CartItem.findById(cartItem._id)
      .populate("variantId", "name price color size image");
  ...
  async removeCartItem(req, res) {
    try {
      ...
      cart.cartItems = cart.cartItems.filter(id => id.toString() !== cartItemId);
